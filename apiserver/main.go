...
import (
	...
	"flag"
	"fmt"
	"github.com/arnab-baishnab/learning-extended-api-server/lib/certstore"
	"github.com/arnab-baishnab/learning-extended-api-server/lib/server"
...
func main() {
	...
	clientCert, clientKey, err := store.NewClientCertPair(cert.AltNames{
		DNSNames: []string{"arnab"},
	})
	if err != nil {
		log.Fatal(err)
	}
	err = store.Write("arnab", clientCert, clientKey)
	if err != nil {
		log.Fatal(err)
	}
